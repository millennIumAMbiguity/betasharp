...
internal class NetherChunkGenerator : CommonChunkGenerator, IChunkSource
{
    ...
    public Chunk GetChunk(int chunkX, int chunkZ)
    {
        ...
        byte[] blocks = new byte[_world.Properties.ChunkSize];
            ...
            for (int sampleZ = 0; sampleZ < horiScale; ++sampleZ)
            {
                for (int sampleY = 0; sampleY < 16; ++sampleY)
                {
                    ...
                    for (int subY = 0; subY < 8; ++subY)
                    {
                        ...
                        {
                            int blockIndex = _world.WorldChuckFormat.GetIndex(subX + sampleX * 4, sampleY * 8 + subY, sampleZ * 4);
                            ...
                            for (int subZ = 0; subZ < 4; ++subZ)
                            {
                                ...
                                blockIndex += _world.WorldChuckFormat.Z1;
