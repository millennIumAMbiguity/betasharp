using java.lang;
using java.util.regex;

namespace betareborn.Chunks
{
    public class ChunkFile : java.lang.Object, Comparable
    {
        private readonly java.io.File field_22326_a;
        private readonly int field_22325_b;
        private readonly int field_22327_c;

        public ChunkFile(java.io.File var1)
        {
            field_22326_a = var1;
            Matcher var2 = ChunkFilePattern.field_22189_a.matcher(var1.getName());
            if (var2.matches())
            {
                field_22325_b = Integer.parseInt(var2.group(1), 36);
                field_22327_c = Integer.parseInt(var2.group(2), 36);
            }
            else
            {
                field_22325_b = 0;
                field_22327_c = 0;
            }

        }

        public int func_22322_a(ChunkFile var1)
        {
            int var2 = field_22325_b >> 5;
            int var3 = var1.field_22325_b >> 5;
            if (var2 == var3)
            {
                int var4 = field_22327_c >> 5;
                int var5 = var1.field_22327_c >> 5;
                return var4 - var5;
            }
            else
            {
                return var2 - var3;
            }
        }

        public java.io.File func_22324_a()
        {
            return field_22326_a;
        }

        public int func_22323_b()
        {
            return field_22325_b;
        }

        public int func_22321_c()
        {
            return field_22327_c;
        }

        public int CompareTo(object? var1)
        {
            return func_22322_a((ChunkFile)var1!);
        }
    }

}